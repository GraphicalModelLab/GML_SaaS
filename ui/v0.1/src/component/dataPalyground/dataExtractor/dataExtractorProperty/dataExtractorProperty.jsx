...
export default class DataExtractorProperty extends React.Component<Props, {}> {
  ...
  render() {
    return <div className={ styles.nodeProp }>
             ...
             <ReactAutocomplete ref="propName" className={ styles.nodePropName } items={ this.state.propItems } shouldItemRender={ (item, value) => item.label.toLowerCase().indexOf(value.toLowerCase()) > -1 } getItemValue={ item => item.label }
               menuStyle={ menuStyleContent } renderItem={ (item, highlighted) => <div key={ item.id } style={ { backgroundColor: highlighted ? '#eee' : 'transparent', "zIndex": 2000 } }>
                                                                                    { item.label }
                                                                                  </div> } value={ this.state.propAutoCompletedValue } onChange={ e => this.setState({
